...
'''
Temp Solution:
USED THE FOLLOWING COMMANDS TO MOVE SOME OF THE FILES TO OTHER DIRECTORIES
ls -1 | wc -l

15% of the total was 405

ls -1 | head -n 405 > movelist.txt

xargs mv -t /blue/fl-poly/oguzhan/radimagenet/data/organized-train-val/FLAIR/val/1 < movelist.txt
'''
...
def save_fig_old(img, path, imageName, tight_layout=True, fig_extension="png", resolution=300):
...
def save_fig(img, path, imageName, fig_extension="png"):
    path.mkdir(parents=True, exist_ok=True)
    imageName = imageName + fig_extension
    imagePath = path / imageName
...
def save(parent, name, list, suffixes, as_png=False):
    for i in range(len(list)):
        ...
        if not as_png:
            ...
            img = Image.fromarray((a1[0] * 255).astype('uint8'))
            spath = str(spath) + ".png"
            print(f"{spath} - Writing")
            img.save(spath)
...
# original data: /blue/fl-poly/oguzhan/radimagenet/data/train
# cleaned data: /blue/fl-poly/oguzhan/radimagenet/data/cleaned
newData = Path('/blue/fl-poly/oguzhan/radimagenet/data/cleaned')
#/blue/fl-poly/oguzhan/radimagenet/data/train
#./brain-tumor-target/train

#processed = Path("./brain-tumor-target/preprocessed")
processed = Path("/blue/fl-poly/oguzhan/radimagenet/data/img-reorganized")
processed.mkdir(parents=True,exist_ok=True)
...
csv_dir = "/blue/fl-poly/oguzhan/radimagenet/data/train_labels.csv"
...
patientdirs = natsorted(list(newData.iterdir()))
...
def main():
    ...
    histogramFile = open("ResizeHistogram.txt", "w")
    ...
    for patient in test:
        ...
        for type in types:
            ...
            if (newData / patient.name / type).exists():
                typeArraysList = transformationPipeline(natsorted((newData / patient.name / type).iterdir()), histogramFile, test=True)
            else:
                print(f"{patient.name} - {type}: DNE")
                continue
            saveTest(labelDir, patient.name, typeArraysList, saveAsPng)
    ...
    for patient in train:
        ...
        for type in types:
            ...
            """
            if (newData / patient.name / type).exists():
                typeArraysList = transformationPipeline(natsorted((newData / patient.name / type).iterdir()), histogramFile)
            else:
                typeArraysList = [np.zeros((240, 240)) for i in range(9)]
            save(labelDir, patient.name, typeArraysList, suffixes, saveAsPng)
            print(f"{patient.name} - {type}: Done")
